...
with open('Giant_393212_entry_E_field_notransferfunc.txt', 'rb') as handle:
    eField1= handle.read()
...
grids['nZ'] = 100
grids['dw'] = (2*np.pi) / (grids['nPts'] * 1.37*u.fs)#grids['dt']) #EQUAL TO (self.input_pk['freq_vector'][2] - self.input_pk['freq_vector'][1])*(2*np.pi)
